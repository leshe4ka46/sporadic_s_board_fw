...
void quat2Euler( float q[4], float e[3] ) {
  ...
  double sqy = q[2] * q[2];
  double sqz = q[3] * q[3];
  e[0] = atan2f(2.f * (q[2] * q[3] + q[1] * q[0]), 1 - 2.f * (sqx + sqy))*57.295779512896175; // -sqx - sqy + sqz + sqw);
  e[1] = asinf(-2.f * (q[1] * q[3] - q[2] * q[0]))*57.295779512896175;
  e[2] = atan2f(2.f * (q[1] * q[2] + q[3] * q[0]), 1 - 2.f * (sqy + sqz))*57.295779512896175; //sqx - sqy - sqz + sqw);
